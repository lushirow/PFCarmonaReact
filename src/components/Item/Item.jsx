...
function Item({ item, stock, name, price, descripcion, picture, category } ) {
  const { getItemQuantity, isInCart } = useCartContext();


  return (
    <Card key={item.id} className="bg back h-100 shadow-lg p-3 mb-3 mr-2 ml-2 rounded">
    <Card.Title>{category}/{name} </Card.Title>
    <span className="position-absolute top-4 end-4 translate-middle badge rounded-pill bg">{isInCart(item) ?
...
<LinkContainer to={`/item/${item}`}><Card.Img className="picHover" variant="top" src={picture} alt={name} /></LinkContainer>
    ...
    <LinkContainer to={`/item/${item}`}><Button className="btn">Ver detalles</Button></LinkContainer>
  </Card>
